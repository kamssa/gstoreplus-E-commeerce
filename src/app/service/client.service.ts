...
// observables streams
  ...
  registraction(client: Client): Observable<Resultat<Client>> {
    console.log('methode du service qui ajoute un client', client);
    return this.http.post<Resultat<Client>>(`${environment.apiUrl}/api/auth/signupc/`, client);
  }
  getClientByEmail(email: string): Observable<Resultat<Client>> {
    return this.http.get<Resultat<Client>>(`${environment.apiUrl}/api/auth/getClient/${email}`);
  }
